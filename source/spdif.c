...
struct SpdifBitstreamAnalyzer
{
    ...
    #define SPDIF_ANALYZER_SAMPLE_EDGES (1<<5)
...
#define CHANNEL_STATUS_NBYTES   (CHANNEL_STATUS_NBITS>>3)
    uint32_t        channel_status_left_bits;
    uint32_t        channel_status_right_bits;
    ...
    float                         threshold_23 )
{
    ...
    i = (uint64_t) sba->r_edgenum & SPDIF_ANALYZER_EDGE_MASK;
...
//        printf("%ld,%ld\n", sba->w_edgenum, sba->r_edgenum );
        ...
        if ( sba_AnalyzeRecentEdges(sba, &threshold_12, &threshold_23 ) )
        {
            ...
            dt23 = threshold_23 - sba->last_threshold_23;
            if ( dt23 < 0 )     
                dt23 = -dt23;
            ...
            if (sba->edge[sba->r_edgenum & SPDIF_ANALYZER_EDGE_MASK].dt > threshold_23 * 2)
                sba->edge[sba->r_edgenum & SPDIF_ANALYZER_EDGE_MASK].dt = threshold_23 - 1;

            if (sba->edge[sba->w_edgenum & SPDIF_ANALYZER_EDGE_MASK].dt > threshold_23 * 2)
                sba->edge[sba->w_edgenum & SPDIF_ANALYZER_EDGE_MASK].dt = threshold_23 - 1;
            ...
            if ( 0 != (synctype = sba_FindSync( sba, threshold_12, threshold_23 )) )
            {
                ...
                sba->sync_start_time = sba->edge[ sba->r_edgenum & SPDIF_ANALYZER_EDGE_MASK ].t - sba->edge[sba->r_edgenum & SPDIF_ANALYZER_EDGE_MASK].dt;

                sba_ReadSample( sba, synctype, threshold_12 );

                sba->sync_end_time = sba->edge[ sba->r_edgenum & SPDIF_ANALYZER_EDGE_MASK ].t - sba->edge[sba->r_edgenum & SPDIF_ANALYZER_EDGE_MASK].dt;
...
static void print_sample   ( void *userdata, uint64_t t, uint64_t tend, enum SpdifFrameType ft, uint32_t aud_sample )
{
    ...
    if ( NULL != sba->wout )
    {
         ...
         */
#if 1
        pcmval = (uint64_t)((aud_sample & 0x0ffff000) >> 12);
#else
        pcmval = (((uint64_t)sba->sample[3] & 0x0f) << 12) | 
                 (((uint64_t)sba->sample[2] & 0xff) << 4) | 
                 (((uint64_t)sba->sample[1] & 0xf0) >> 4);
#endif
